 ...
 * v 1.5 (17.02.2020) оновлено __construct($request), private $amp
 */
...
class Data {
	...
	public function array_language(Array $where = [])
	{
		if(LANGUAGE)
			return $where + ['language' => LANGUAGE];
		return $where;
	}

	// get text from serialized array in multilanguage mode
	public function serialized_language($text)
	{
		if (!LANGUAGE)
			return $text;
	...
	public function get_file_path($media, $prefix_filename = '', $folder = 'media', $sub_folder = '')
	{
		$alias_folder = $this->alias->alias;
		if (!empty($media->alias_folder))
			$alias_folder = $media->alias_folder;
		if(!empty($sub_folder))
			$sub_folder .= DIRSEP;
		$depth = 2;
		if ($media->content_id < 0)
			$depth = 1;
		return $folder . DIRSEP . $alias_folder . DIRSEP . $sub_folder . $this->get_subpathByContent($media->content_id, $depth) . DIRSEP . $prefix_filename . $media->file_name;
	}

	public function get_subpathByContent($content = 0, $depth = 1)
	{
		if ($depth == 0 || $content == 0 || !is_numeric($content))
			return $content;
		$p_100 = ceil($content / 100) * 100;
		if ($depth == 1)
			return $p_100 . DIRSEP . $content;
		// $depth == 2
		$p_1000 = ceil($content / 1000) * 1000;
		return $p_1000 . DIRSEP . $p_100 . DIRSEP . $content;
	}
